...
import { UserCheck, MapPin, Calendar, ExternalLink, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <Card className="p-4 sm:p-10 bg-white dark:bg-[#0f172a] border border-slate-100 dark:border-slate-800 rounded-3xl shadow-2xl shadow-slate-200/40 dark:shadow-none mt-6 sm:mt-10 transition-all duration-500 overflow-hidden">
      ...
      <div className="border border-slate-50 dark:border-slate-800/60 rounded-2xl overflow-hidden">
        <Table className="bg-transparent">
          ...
          <TableBody>
            {data.map((item) => (
              <TableRow key={item.name} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors border-b border-slate-50 dark:border-slate-800/40 last:border-none">
                <TableCell className="py-4 sm:py-6 pl-4 sm:pl-8">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-700 flex items-center justify-center text-slate-500 font-black text-[10px] sm:text-xs">
                        {item.name.split(" ").map(n => n[0]).join("").slice(-2).toUpperCase()}
                    </div>
                    <div>
                      <Text className="font-bold text-slate-700 dark:text-slate-200 text-xs sm:text-base">{item.name}</Text>
                      <Text className="sm:hidden text-[10px] text-slate-400 font-medium">{item.branch}</Text>
                    </div>
                  </div>
                </TableCell>
                ...
                <TableCell className="text-center">
                  <div className="flex flex-col items-center">
                    <Text className="text-xs sm:text-sm font-bold text-slate-800 dark:text-white">{item.time}</Text>
                    <Text className="hidden sm:block text-[10px] text-slate-400 font-medium">{item.date}</Text>
                  </div>
                </TableCell>
                <TableCell className="hidden lg:table-cell">
                  <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400">
                    <MapPin className="w-3 h-3 text-rose-500" />
                    <span className="text-xs font-medium tracking-tight truncate max-w-[200px]">{item.location}</span>
                  </div>
                </TableCell>
      ...
      <Flex justifyContent="between" alignItems="center" className="mt-8 sm:mt-10 px-0 sm:px-2 flex-col sm:flex-row gap-4">
        <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hiển thị 5 trên 1.250 bản ghi</Text>
